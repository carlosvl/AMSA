#!/usr/bin/env python3
"""
Ghost Mexico_Seminars__c Record Detector and Cleaner

Detects and optionally deletes "ghost" Mexico_Seminars__c records that are
connected to a Contact (Applicant__c) but lack meaningful data.

Two categories:
  Category A: Missing key lookup (Symposium__c is NULL)
  Category B: Fully empty ghosts (ALL data fields are NULL/empty except Applicant__c and auto-fields)

Usage:
  python3 delete_ghost_mexico_seminars.py <org_alias> [--dry-run] [--execute] [--category A|B|both]

Examples:
  python3 delete_ghost_mexico_seminars.py "AMSA Prod"                   # detect only (dry-run)
  python3 delete_ghost_mexico_seminars.py "AMSA Prod" --execute         # detect and delete
  python3 delete_ghost_mexico_seminars.py "AMSA Prod" --category A      # only NULL Symposium
"""
import argparse
import csv
import json
...
def query_all_records(org_alias: str) -> List[Dict]:
    """Query all Mexico_Seminars__c records with all data fields."""
    ...
    query = f"SELECT {fields} FROM Mexico_Seminars__c"
...
def classify_ghosts(records: List[Dict], category: str) -> Dict[str, List[Dict]]:
    ...
    cat_a = []  # Missing key lookup (Symposium__c is NULL)
    cat_b = []  # Fully empty
    ...
    for rec in records:
        ...
        symposium_null = is_field_empty(rec.get('Symposium__c'))
        all_empty = all(is_field_empty(rec.get(f)) for f in DATA_FIELDS)

        if category in ('A', 'both') and symposium_null:
            cat_a.append(rec)
    ...
    if category in ('A', 'both'):
        results['A'] = cat_a
        print(f"  📊 Category A (NULL Symposium):      {len(cat_a)} records")
...
def save_backup(records_to_delete: List[Dict], timestamp: str) -> Path:
    ...
    json_path = BACKUP_DIR / f'backup_mexico_seminars_{timestamp}.json'
    json_path.write_text(json.dumps({
        'backup_type': 'pre_deletion_backup',
        'object': 'Mexico_Seminars__c',
        'timestamp': timestamp,
        'record_count': len(records_to_delete),
        'records': records_to_delete,
    }, indent=2, default=str))

    csv_path = BACKUP_DIR / f'backup_mexico_seminars_{timestamp}.csv'
...
def delete_records_bulk(org_alias: str, record_ids: List[str], batch_size: int = 200) -> Dict:
    ...
    for i in range(0, len(record_ids), batch_size):
        ...
        try:
            ...
            # Check for command-level error (no result/jobInfo in response)
...
def generate_report(ghost_results: Dict, org_alias: str) -> str:
    ...
    lines = [
        "=" * 80, "GHOST MEXICO SEMINARS RECORDS REPORT", "=" * 80, "",
        f"Org: {org_alias}",
        f"Report Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", "",
        "=" * 80, "SUMMARY", "=" * 80,
    ]
    for cat, recs in ghost_results.items():
        label = "NULL Symposium" if cat == 'A' else "Fully empty ghosts"
...
def main():
    parser = argparse.ArgumentParser(description='Detect and delete ghost Mexico_Seminars__c records')
    ...
    print("=" * 80)
    print("👻 GHOST MEXICO SEMINARS RECORD DETECTOR")
    print("=" * 80)
    print(f"\n📋 Org: {args.org_alias}")
    ...
    if not records:
        print("⚠️  No Mexico_Seminars__c records found")
        sys.exit(0)
    ...
    report_path = RESULTS_DIR / f'ghost_mexico_seminars_{timestamp}.txt'
    ...
    if args.export_csv:
        for cat, recs in ghost_results.items():
            if not recs:
                continue
            csv_path = RESULTS_DIR / f'ghost_mexico_seminars_cat{cat}_{timestamp}.csv'
            with open(csv_path, 'w', newline='') as f:
                writer = csv.writer(f)
    ...
    run_id = db_utils.create_comparison_run(
        run_type='ghost_mexico_seminar_detection',
    ...
    if dry_run:
        ...
        print(f"  python3 delete_ghost_mexico_seminars.py \"{args.org_alias}\" --category {args.category} --execute")
        return
    ...
    if result['errors']:
        error_path = RESULTS_DIR / f'ghost_mexico_seminar_errors_{timestamp}.json'
