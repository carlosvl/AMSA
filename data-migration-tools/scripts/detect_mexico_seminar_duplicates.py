#!/usr/bin/env python3
"""
Mexico_Seminars__c Duplicate Detection Tool
Detects duplicate Mexico_Seminars__c records within a single Salesforce org.

Duplicate key: Applicant__c + Symposium__c.

Usage:
    python3 detect_mexico_seminar_duplicates.py <org_alias> [--export-json] [--export-csv]

Example:
    python3 detect_mexico_seminar_duplicates.py "AMSA Prod"
    python3 detect_mexico_seminar_duplicates.py "AMSA Prod" --export-json
"""
import json
import subprocess
import sys
...
RUN_TYPE = 'mexico_seminars__c_duplicate_detection'
...
def query_all_records(org_alias):
    """Query all Mexico_Seminars__c records."""
    ...
    query = """
        ...
        ORDER BY Applicant__c, Symposium__c
    """
...
def build_duplicate_key(record):
    """Build composite key: Applicant + Symposium."""
    applicant = record.get('Applicant__c') or 'NULL'
    symposium = record.get('Symposium__c') or 'NULL'
    return f"{applicant}|{symposium}"


def detect_duplicates(records):
    """Group records by key and find groups with more than one member."""
    print(f"\n🔍 Analyzing {len(records)} Mexico_Seminars__c records for duplicates...")
    ...
    for key, recs in groups.items():
        if len(recs) > 1:
            ...
            duplicate_groups[key] = {
                ...
                'symposium': recs[0].get('Symposium__c'),
                'records': recs,
                'count': len(recs),
            }
...
def generate_report(duplicate_groups, org_alias):
    ...
    lines = [
        "=" * 80,
        "MEXICO SEMINARS DUPLICATE DETECTION REPORT",
        "=" * 80, "",
        f"Org: {org_alias}",
        ...
        f"Unique Mexico Seminars Affected: {total_groups}", "",
    ]
    ...
    if duplicate_groups:
        ...
        for idx, group in enumerate(sorted_groups[:50], 1):
            lines.append(f"Group {idx}: Applicant={group['applicant']}  Symposium={group.get('symposium', 'N/A')}")
            ...
            for rec in group['records']:
                ...
                lines.append(f"      App Date: {rec.get('App_Date__c', 'N/A')}")
...
def main():
    parser = argparse.ArgumentParser(description='Detect duplicate Mexico_Seminars__c records')
    ...
    print("=" * 80)
    print("🔍 MEXICO SEMINARS DUPLICATE DETECTION TOOL")
    print("=" * 80)
    print(f"\n📋 Org: {args.org_alias}")
    print(f"   Matching Fields: Applicant__c + Symposium__c\n")
    ...
    if not records:
        print("\n⚠️  No Mexico_Seminars__c records found")
        sys.exit(0)
    ...
    run_id = db_utils.create_comparison_run(
        ...
        notes='Duplicate detection: Applicant + Symposium',
    )
    ...
    if args.export_json:
        path = RESULTS_DIR / f'mexico_seminar_duplicates_{timestamp}.json'
    ...
    if args.export_csv:
        path = RESULTS_DIR / f'mexico_seminar_duplicates_{timestamp}.csv'
        with open(path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Group Key', 'Applicant', 'Symposium', 'Record ID', 'Name',
                             'Stage', 'App Date', 'Created Date', 'Last Modified Date', 'Is Keeper'])
            for gk, gd in duplicate_groups.items():
                for rec in gd['records']:
                    writer.writerow([
                        gk, gd['applicant'], gd.get('symposium', ''),
                        rec['Id'], rec.get('Name', ''), rec.get('Application_Stage__c', ''),
                        rec.get('App_Date__c', ''), rec.get('CreatedDate', ''),
                        rec.get('LastModifiedDate', ''),
                        'Yes' if rec.get('is_keeper') else 'No',
                    ])
        print(f"📄 CSV: {path}")

    report_path = RESULTS_DIR / f'mexico_seminar_duplicates_{timestamp}.txt'
