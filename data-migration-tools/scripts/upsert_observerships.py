#!/usr/bin/env python3
"""
Upsert Observership__c records from source org to target org.

This script:
1. Queries all Observership__c records from source org
2. Maps Applicant__c (Contact) using existing mappings
3. Upserts records to target org using ExternalID__c field

Observership__c has a Master-Detail relationship to Contact (Applicant__c).

Usage:
  python3 upsert_observerships.py <source_org> <target_org> [--dry-run]

Example:
  python3 upsert_observerships.py "AMSA-Royalty-Prod" "AMSA Prod"
"""
...
def query_source_observerships(org_alias: str):
    """Query all Observership__c records from source org."""
    print(f"📥 Querying ALL Observership__c from {org_alias}...")
...
def upsert_records_rest(target_org: str, records: List[Dict], dry_run: bool = False):
    ...
    for i, rec in enumerate(records, 1):
        ...
        url = f"{instance_url}/services/data/v59.0/sobjects/Observership__c/ExternalID__c/{external_id}"
...
def main():
    print("=" * 80)
    print("🏥 UPSERT OBSERVERSHIP RECORDS")
    print("=" * 80)
    print()

    parser = argparse.ArgumentParser(
        description='Upsert Observership__c records from source to target org'
    )
    ...
    source_records = query_source_observerships(source_org)
    ...
    json_path = RESULTS_DIR / f'observership_upsert_{timestamp}.json'
    ...
    if unmapped:
        unmapped_path = RESULTS_DIR / f'observership_unmapped_{timestamp}.json'
